       ...
       //     putrsUART1((ROM char*) "OBTAINED...\r\n");
        ...
        case SM_START_SSL:
			...
			TCPPutString(MySocket, ServerName);            
             if(type == CURRENT_DATA)        TCPPutROMString(MySocket, (ROM BYTE*) "Content-Length: 24\r\n");
             else       TCPPutROMString(MySocket, (ROM BYTE*) "Content-Length: 32\r\n");
			TCPPutROMString(MySocket, (ROM BYTE*)"\r\nConnection: close\r\n\r\n");           
             if(type == CURRENT_DATA)        TCPPutROMArray(MySocket, &WEB_data_0,24);
             else        TCPPutROMArray(MySocket, &History,32);
			// Send the packet
			TCPFlush(MySocket);
			GenericTCPExampleState++;
			break;
